...
async function Availabity(OrImPth: string //OriginalPath
  ...
  try {
    ...
    await (fs.access(`${OrImPth}`, async(Imnotexist:boolean) => {
      // Check the image exists on the original folder
        ...
        ||Number(height)<=0||Number(height)==null)
        {
            res.send('The Image Cannot Be Resized Because Width And Height Should Be Valid Number > 0 ');

        }
        else{
            await ImResF(`OrSImage/${req.params.imagename}`,Number(req.params.width),Number(req.params.height),`Thumbnail/${req.params.width}-${req.params.height}-${req.params.imagename}`);
            {
